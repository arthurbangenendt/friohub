-- ============================================================================
-- BTU recomendado deixa de ser um número que o cliente manda — o banco
-- recalcula a partir dos dados brutos do próprio ambiente (área, pessoas,
-- insolação, andar/telhado, eletrônicos) antes de gravar. Antes disso,
-- `criar_pedido_orcamento` confiava no `btu_recomendado` do payload sem
-- checar se batia com os outros campos do mesmo item — um bug de front-end
-- (ou uma chamada direta à RPC) podia gravar um BTU sem relação com o
-- ambiente descrito, e esse valor errado alimentava a ordenação do
-- catálogo e o contexto do assistente de IA com falsa confiança.
--
-- A fórmula replica exatamente src/lib/btu.ts — mesma regra de mercado, só
-- em SQL. Ver comentário daquele arquivo para a justificativa dos números.
-- ============================================================================

create or replace function public.calcular_btu_recomendado(
  p_area_m2 numeric,
  p_num_pessoas integer,
  p_insolacao_alta boolean,
  p_andar_ou_telhado boolean,
  p_eletronicos integer
)
returns integer
language sql
immutable
...
as $$
  select coalesce(
    (
      select c
        from unnest(array[9000, 12000, 18000, 24000, 30000, 36000]) c
       where c >= round(
         greatest(0, coalesce(p_area_m2, 0))
           * (case when coalesce(p_insolacao_alta, false) or coalesce(p_andar_ou_telhado, false)
                   then 800 else 600 end)
         + greatest(0, greatest(1, coalesce(p_num_pessoas, 1)) - 2) * 600
         + greatest(0, coalesce(p_eletronicos, 0)) * 600
       )
       order by c
       limit 1
    ),
...
-- Bloco interno de criar_pedido_orcamento, não um oráculo público de cálculo.
revoke all on function public.calcular_btu_recomendado(
  numeric, integer, boolean, boolean, integer
) from public, anon, authenticated;

comment on function public.calcular_btu_recomendado(
  numeric, integer, boolean, boolean, integer
) is 'Réplica em SQL de calcularBtu (src/lib/btu.ts) — fonte única do BTU persistido, não confia no valor enviado pelo cliente.';

-- ---------------------------------------------------------------------------
-- criar_pedido_orcamento — mesmo corpo de 20260819200000_inadimplencia_marketplace.sql,
-- com uma mudança: depois de resolver v_itens, cada item tem seu btu_recomendado
-- sobrescrito pelo valor recalculado. v_primeiro e o insert de
-- quote_request_itens já leem de v_itens, então o valor correto se propaga
-- sem tocar no resto da função.
-- ---------------------------------------------------------------------------
...
comment on function public.criar_pedido_orcamento(
  ...
  'aparelho e inadimplência antes de gravar.';
