-- ============================================================================
-- Correção: PAYMENT_RECEIVED travava ao tentar reativar assinatura cancelada
-- ============================================================================
--
-- Bug real encontrado testando em produção: cancelar uma payment_charge/
-- plan_subscription no nosso banco (20260818149000, ao trocar de plano antes
-- de pagar) NÃO cancela a fatura correspondente no Asaas. Quando essa fatura
-- velha acaba sendo paga mesmo assim, o webhook PAYMENT_RECEIVED chega e o
-- código tentava marcar a assinatura cancelada como 'active' de novo —
-- colidindo com `uq_plan_subscriptions_active_professional` porque o
-- profissional já tinha uma assinatura mais nova ocupando aquele lugar.
-- O evento ficava permanentemente em 'error' (erro de constraint, sem retry
-- possível) e o dinheiro nunca era refletido nem no ledger.
--
-- Correção: o lançamento contábil (dinheiro entrou de verdade, é fato) sempre
-- acontece. Só a cascata comercial (ativar assinatura, promover o profissional)
-- passa a exigir que aquela assinatura especificamente ainda não esteja
-- cancelada — nunca ressuscita um compromisso morto. Uma fatura paga presa a
-- uma assinatura cancelada fica como receita registrada sem vínculo comercial
-- ativo; resolver isso (reembolsar ou religar a um profissional) é decisão
-- manual, não automática.
--
-- Fica em aberto (fora desta correção): cancelar a fatura no Asaas de verdade
-- quando cancelamos localmente, para a divergência não acontecer de novo.
...
begin
  ...
  case v_event.event_type
    ...
    when 'PAYMENT_RECEIVED' then
      ...
      -- O dinheiro entrou de verdade: o lançamento acontece sempre, mesmo que
      -- a assinatura por trás já tenha sido cancelada localmente (fatura velha
      -- paga fora de hora). O ledger reflete o fato; a cascata comercial abaixo
      -- é que fica condicional.
      ...
      if v_charge.subscription_id is not null then
        ...
        -- Assinatura já cancelada: o dinheiro fica registrado no ledger, sem
        -- promover ninguém. Resolver (reembolsar ou religar manualmente) é
        -- decisão humana — não existe leitura automática segura daqui.
      end if;
