...
const LABEL: Record<string, string> = {
  ux_pipeline: "Pipeline e confiança",
  ux_execution: "Execução profissional",
  ux_portfolio: "Carteira e recorrência",
  ux_growth: "Gestão e crescimento",
};
  ...
  return (
    <main className="container-tight" style={{ padding: "40px 24px 80px" }}>
      ...
      <div style={{ display: "grid", gap: 14, marginTop: 24 }}>
        {(flags ?? []).map((flag) => {
          const region = Array.isArray(flag.region) ? flag.region[0] : flag.region;
          if (!region) return null;
          return (
            <section key={`${flag.flag_key}-${region.slug}`} className="card" style={{ padding: 20 }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
                <div>
                  <strong>{LABEL[flag.flag_key] ?? flag.flag_key}</strong>
