...
export default async function PerfilPage() {
  ...
  const { data: pro } = await supabase
    .from("professionals")
    .select("tipo, razao_social, bio, cidade, verification_status, professional_skills ( specialty, years_experience ), service_areas ( cep_prefix )")
    .eq("id", user.id)
    .maybeSingle();
  ...
  const inicial: PerfilInput = {
    ...
    bio: pro?.bio ?? "",
    cidade: pro?.cidade ?? "Fortaleza",
    cepPrefix: (pro?.service_areas?.[0]?.cep_prefix as string) ?? "60",
