...
const status: Record<string, string> = { requested: "Aguardando atribuição", proposed: "Aguardando cliente", offered: "Aguardando profissional", active: "Ativo", paused: "Pausado", cancelled: "Cancelado" };
...
export default async function PmocPage() {
  ...
  const { data: equipamentos } = papel !== "profissional"
  ...
  return <div style={wrap}>
    ...
    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 20 }}>
      {papel === "admin" && <Link href="/admin/pmoc" className="btn">Abrir fila administrativa</Link>}
      {papel === "profissional" && <Link href="/painel/pmoc/propor" className="btn btn-primary">Propor PMOC a um cliente</Link>}
    ...
    <div style={{ display: "grid", gap: 12 }}>{planos.map((plano) => <PlanoCard key={plano.id} plano={plano} isPro={papel === "profissional"} equipamentos={equipamentos ?? []} vinculados={(vinculos ?? []).filter((v) => v.plan_id === plano.id).map((v) => v.equipment_id)} />)}</div>
  </div>;
}

function PlanoCard({ plano, isPro, equipamentos, vinculados }: { plano: Plano; isPro: boolean; equipamentos: { id: string; brand: string | null; model: string | null }[]; vinculados: string[] }) {
  ...
  return <article className="card" style={{ padding: 20 }}>
    ...
    {!isPro && equipamentos.length > 0 && <form action={vincularEquipamentoPmoc} style={{ display: "flex", gap: 8, flexWrap: "wrap", margin: "12px 0" }}><input type="hidden" name="planoId" value={plano.id}/><select name="equipamentoId" defaultValue="" required style={{ padding: 8, border: "1px solid var(--line)", borderRadius: 8 }}><option value="" disabled>Vincular equipamento</option>{equipamentos.filter((e) => !vinculados.includes(e.id)).map((e) => <option key={e.id} value={e.id}>{[e.brand,e.model].filter(Boolean).join(" ") || "Equipamento"}</option>)}</select><button className="btn">Vincular ao PMOC</button></form>}
